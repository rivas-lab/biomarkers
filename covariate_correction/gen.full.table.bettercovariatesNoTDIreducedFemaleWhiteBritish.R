...
full.cov <- read.table("covariates/merged.phe", header=T, stringsAsFactors=F) %>%
    ...
    left_join(batch) %>% left_join(eth) %>% left_join(anth) %>% filter(sex==0, Ethnicity == "1001") %>% select(-sex, -Ethnicity)
...
results <- sapply(1:nrow(biomarker2day), function(x) process.phe(biomarker2day$name[x], biomarker2day$biomarker[x], "", paste0("outputExtendedBMIreducedFemaleWhiteBritish/phenotypes/residual/", biomarker2day$name[x], ".phe"), paste0("outputExtendedBMIreducedFemaleWhiteBritish/phenotypes/invnorm/%s/", biomarker2day$name[x], ".phe")))

dir.create("outputExtendedBMIreducedFemaleWhiteBritish/phenotypes", recursive=T)
dir.create("outputExtendedBMIreducedFemaleWhiteBritish/covariates", recursive=T)
save(results, file="outputExtendedBMIreducedFemaleWhiteBritish/covariates/regressions.RData")
...
results.statins <- sapply(1:nrow(biomarker2day), function(x) process.phe(biomarker2day$name[x], biomarker2day$biomarker[x], "_adjstatins", paste0("outputExtendedBMIreducedFemaleWhiteBritish/phenotypes/residual/", biomarker2day$name[x], ".adjust.statins.phe"), paste0("outputExtendedBMIreducedFemaleWhiteBritish/phenotypes/invnorm/%s/", biomarker2day$name[x], ".adjust.statins.phe")))

save(results.statins, file="outputExtendedBMIreducedFemaleWhiteBritish/covariates/regressions.adjust.statins.RData")

write.table(regression.data, "outputExtendedBMIreducedFemaleWhiteBritish/phenotypes/full.table.phe", quote=F, sep="\t", row.names=F, col.names=T)
