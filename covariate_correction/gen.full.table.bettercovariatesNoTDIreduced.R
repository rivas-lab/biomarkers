...
kept.terms <- colnames(full.cov %>% select(-IID, -FID, -age, -Array, -TDI))
...
results <- sapply(1:nrow(biomarker2day), function(x) process.phe(biomarker2day$name[x], biomarker2day$code[x], "", paste0("outputExtendedNoTDIreduced/phenotypes/residual/", biomarker2day$name[x], ".phe"), paste0("outputExtendedNoTDIreduced/phenotypes/invnorm/%s/", biomarker2day$name[x], ".phe")))

dir.create("outputExtendedNoTDIreduced/phenotypes", recursive=T)
dir.create("outputExtendedNoTDIreduced/covariates", recursive=T)
save(results, file="outputExtendedNoTDIreduced/covariates/regressions.RData")
...
results.statins <- sapply(1:nrow(biomarker2day), function(x) process.phe(biomarker2day$name[x], biomarker2day$code[x], "_adjstatins", paste0("outputExtendedNoTDIreduced/phenotypes/residual/", biomarker2day$name[x], ".adjust.statins.phe"), paste0("outputExtendedNoTDIreduced/phenotypes/invnorm/%s/", biomarker2day$name[x], ".adjust.statins.phe")))

save(results.statins, file="outputExtendedNoTDIreduced/covariates/regressions.adjust.statins.RData")

write.table(regression.data, "outputExtendedNoTDIreduced/phenotypes/full.table.phe", quote=F, sep="\t", row.names=F, col.names=T)
